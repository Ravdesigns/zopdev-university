/* ============================================================================
   foot-globe.js · Footer interactive world map (dotted land + cursor push +
   drop-a-pin + arc fan). Ported from the zop.dev marketing footer. Targets
   <canvas id="foot-regions-map"> inside <div id="foot-regions-v2">.
   Self-initializing; early-returns when the canvas/container are absent, so
...
(function(){
  ...
  var regions = [
    {lat:38.9,  lon:-77.0},{lat:45.5,  lon:-122.7},{lat:-23.5, lon:-46.6},
    {lat:51.5,  lon:-0.12},{lat:50.1,  lon:11.5},{lat:26.0,  lon:50.5},
    {lat:19.1,  lon:75},{lat:1.35,  lon:103.8},{lat:35.7,  lon:139.7},{lat:-33.9, lon:151.2}
  ];

  function project(lat, lon){ return { x: (lon + 180) / 360 * W, y: (78 - lat) / 150 * H }; }
  ...
  function qBezier(pa, ctrl, pb, t){
    var it = 1 - t;
    return { x: it*it*pa.x + 2*it*t*ctrl.x + t*t*pb.x, y: it*it*pa.y + 2*it*t*ctrl.y + t*t*pb.y };
  }
  ...
  function onCursorMove(){
    ...
    var lat = 78 - my / H * 150;
    if(!isLand(lat, lon)){ hideGhost(); return; }
    showGhost(mx, my);
  }
  function fireArcs(e){
    ...
    setTimeout(function(){ if(ghostEl) ghostEl.classList.remove('flash'); }, 360);
    var picks = [];
  ...
  container.addEventListener('mouseleave', function(){ mx = -9999; my = -9999; hideGhost(); });
