# Reference Library

The reference library is the cross-cutting resource for ZopDev University learners.

## Structure

```
reference/
├── rules/           460 recommendation rules, indexed by ID
├── glossary/        ~120 terms, A-Z
├── patterns/        15 CDCR patterns with worked examples
└── worked_examples/ ~25 cost calculations and case studies
```

## How to use

```
Need a definition?            → glossary/
Need a rule's evidence/fix?    → rules/RC-XXX.md
Need a tested cost pattern?    → patterns/
Need a worked calculation?    → worked_examples/
```

## What's here

- [Rule catalog](rules/00_README.md) — 460 rules across 3 providers
- [Glossary](glossary/00_README.md) — A-Z definitions
- [CDCR Patterns](patterns/00_README.md) — 15 production-grade patterns
- [Worked Examples](worked_examples/00_README.md) — cost math case studies

---

§ Last reviewed 2026-05-20
