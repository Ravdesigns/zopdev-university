...
AWS:
  ...
  Tag Policies (Orgs)   → govern allowed tag keys/values org-wide
                          (SCPs are permission guardrails, not tag appliers)
  Resource-level        → explicit; tags do not auto-inherit on AWS
  
GCP:
  Labels do NOT inherit  → folder/project labels do not flow to resources
  Resource Manager Tags  → DO inherit (folder → project → resource)
  Cost-label consistency → comes from IaC default_labels, not inheritance
...
GCP:
  Labels do NOT inherit; set them via IaC default_labels per resource
  Some legacy services (e.g., BigQuery datasets) handle labels differently
  GCP labels never inherit; only Resource Manager Tags do (folder->project->resource)
